...
- `internal/render/shell.go` — pins row 1 via DECSTBM scroll margins (`ESC[2;<LINES>r`); **every margin set is paired with a guaranteed reset** on exit/disable or terminals (ConPTY) corrupt. Synchronized output, wcwidth-ish truncation, OSC-8 hyperlink, theme-neutral palette styling.
