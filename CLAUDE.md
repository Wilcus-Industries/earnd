...
- `cmd/earnd/main.go` — subcommands: `render` (hot path: draw cached creative on row 1, spawn detached `tick`), `tick` (background state machine: begin/heartbeat/redeem), `register`, `open` (browser → click URL, safety-checked), `on`/`off`, `reset` (release margins on shell EXIT), `clear` (preserve banner), `status`, `version`.
- `internal/core/session.go` — tick lifecycle state machine; rotates impression token before its 120s server TTL (`tokenMaxAgeSeconds=110`); creative cached atomically to `creative.json`.
...
- `internal/config/config.go` — state under `$XDG_CONFIG_HOME/earnd` (or `~/.config/earnd`): `state.json` (on/off), `creative.json`, `online.flag`, `device.key` (0600), `device.json` (0600), `session.json`, `tick.lock`. `SecureBase()` enforces https for non-loopback hosts (plaintext http only for localhost).
